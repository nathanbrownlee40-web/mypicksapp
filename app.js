...
// Keep an immutable copy so we can restore after imports
const DEFAULT_DATASETS = JSON.parse(JSON.stringify(DATASETS));
...
// ---- Imports persistence (local) ----
const IMPORT_KEY_PREFIX = "picks_import_ds_v1_";

function saveImported(slug, dataset){
  try{
    const payload = { columns: dataset.columns, rows: dataset.rows, probCol: dataset.probCol, name: dataset.name, slug: dataset.slug };
    localStorage.setItem(IMPORT_KEY_PREFIX + slug, JSON.stringify(payload));
  }catch(e){}
}
function loadImported(slug){
  try{
    const raw = localStorage.getItem(IMPORT_KEY_PREFIX + slug);
    if(!raw) return null;
    const obj = JSON.parse(raw);
    if(!obj || !Array.isArray(obj.rows) || !Array.isArray(obj.columns)) return null;
    return obj;
  }catch(e){ return null; }
}
function clearImported(slug){
  try{ localStorage.removeItem(IMPORT_KEY_PREFIX + slug); }catch(e){}
}

function restoreDefaultDataset(slug){
  const def = DEFAULT_DATASETS.find(d=>d.slug===slug);
  const idx = state.datasets.findIndex(d=>d.slug===slug);
  if(!def || idx < 0) return false;
  state.datasets[idx] = JSON.parse(JSON.stringify(def));
  clearImported(slug);
  return true;
}
...
function openDetails(row){
  // Keep a pointer for delete (Imported only)
  state.detailRowId = row?.__id ?? null;
  const rmBtn = $("removeRowBtn");
  if(rmBtn){
    rmBtn.classList.toggle("hidden", state.current?.slug !== "imported");
  }
  ...
  Object.keys(row).filter(k=>!String(k).startsWith("__")).forEach(k=>{
    const kk=document.createElement("div"); kk.className="k"; kk.textContent=k;
...
async function init(){
  ...
  $("removeRowBtn")?.addEventListener("click", ()=>{
    if(state.current?.slug !== "imported") return;
    const id = state.detailRowId;
    if(!id) return;
    if(!confirm("Remove this game from Imported?")) return;
    const dsIdx = state.datasets.findIndex(d=>d.slug==="imported");
    if(dsIdx<0) return;
    const ds = state.datasets[dsIdx];
    ds.rows = (ds.rows||[]).filter(r=>r.__id !== id);
    ds.columns = ds.columns || Object.keys(ds.rows[0]||{});
    saveImported("imported", ds);
    loadDataset("imported");
    try{ $("details").close(); }catch(e){}
  });
  ...
  // Apply any saved imported datasets
  state.datasets = state.datasets.map(ds=>{
    if(ds.slug === \"tracker\" || ds.slug === \"accas\") return ds;
    const imp = loadImported(ds.slug);
    return imp ? Object.assign({}, ds, imp) : ds;
  });
...
/***********************
 * CSV Import (File OR Google Sheet export)
 * - Choose destination dashboard (Over 2.5 / BTTS / Over 1.5 / Imported tab)
 * - Parses numbers so badges/stats work
 * - Can restore defaults / clear imported
 ***********************/
(function setupCsvImport(){
  ...
  const DASH_SLUGS = ["over25","btts-yes","over15"];
  ...
  function coerceValue(key, val, probCol){
    ...
    // Percent strings -> 0-1
    if(s.endsWith("%") && isNumLike(s)){
      const n = parseFloat(s.slice(0,-1));
      return Number.isFinite(n) ? (n/100) : s;
    }
    ...
    if(isNumLike(s)){
      ...
      // Probability columns in this app are 0-1. If the CSV gives 70 (meaning 70%),
      // convert to 0.70 for the prob column only.
      const isProb = (key === probCol) || /^p\(/i.test(key) || key.toLowerCase().includes("prob");
      if(isProb && n > 1 && n <= 100) return n/100;
  ...
  function pickDestination(){
    ...
    if(choice === "1") return "over25";
    if(choice === "2") return "btts-yes";
    if(choice === "3") return "over15";
    if(choice === "4") return "imported";
    return null;
  }

  function applyImportedRows(targetSlug, headers, rawRows){
    ...
    // Determine which prob column should be used for this target
    let probCol = null;
    if(targetSlug === "over25") probCol = "P(Over2.5)";
    if(targetSlug === "btts-yes") probCol = "P(BTTS)";
    if(targetSlug === "over15") probCol = "P(Over1.5)";

    // If importing to a new tab, auto-detect a prob column if possible
    if(targetSlug === "imported"){
      probCol = ["P(Over2.5)","P(BTTS)","P(Over1.5)","P"].find(c=> headers.includes(c)) || null;
    }

    // Coerce values (numbers + prob)
    rows.forEach(r=>{
      Object.keys(r).forEach(k=>{
        r[k] = coerceValue(k, r[k], probCol);
      });
    });
    ...
    let dataset;
    if(targetSlug === "imported"){
      dataset = { slug:"imported", name:"Imported", columns:[...headers], rows, probCol };
      const existingIdx = state.datasets.findIndex(d => d.slug === "imported");
      if(existingIdx >= 0) state.datasets[existingIdx] = dataset;
      else state.datasets.splice(Math.max(0, state.datasets.length-2), 0, dataset); // before tracker/accas
      saveImported("imported", dataset);
      buildTabs();
      loadDataset("imported");
      return;
    }

    // Import into an existing dashboard: replace its rows/columns, keep its probCol/name/slug
    const idx = state.datasets.findIndex(d=> d.slug === targetSlug);
    if(idx < 0) return;

    dataset = Object.assign({}, state.datasets[idx], {
      columns: [...headers],
      rows
    });

    // Ensure probCol is set for dashboard
    dataset.probCol = probCol;

    state.datasets[idx] = dataset;
    saveImported(targetSlug, dataset);
    buildTabs();
    loadDataset(targetSlug);
  }
  ...
  async function importCsvText(text){
    ...
    if(!headers.length) throw new Error("Missing headers row");

    const target = pickDestination();
    if(!target) return;

    applyImportedRows(target, headers, body);
    alert(`Imported ${body.length} rows ✅`);
  }

  function clearMenu(){
    const choice = prompt(
      "Remove imported data:\n" +
      "1 = Restore Over 2.5 defaults\n" +
      "2 = Restore BTTS defaults\n" +
      "3 = Restore Over 1.5 defaults\n" +
      "4 = Delete 'Imported' tab\n\n" +
      "Enter 1-4:"
    );
    if(choice === "1"){ restoreDefaultDataset("over25"); buildTabs(); loadDataset("over25"); return; }
    if(choice === "2"){ restoreDefaultDataset("btts-yes"); buildTabs(); loadDataset("btts-yes"); return; }
    if(choice === "3"){ restoreDefaultDataset("over15"); buildTabs(); loadDataset("over15"); return; }
    if(choice === "4"){
      const idx = state.datasets.findIndex(d=>d.slug==="imported");
      if(idx>=0) state.datasets.splice(idx,1);
      clearImported("imported");
      buildTabs();
      loadDataset(state.datasets[0]?.slug || "over25");
      return;
    }
  }
  ...
  input.addEventListener("change", (e)=>{
    ...
    reader.onload = ()=> {
      importCsvText(String(reader.result||"")).catch(err=>{
        console.error(err);
  ...
  btn.addEventListener("click", async ()=>{
    const choice = prompt(
      ...
      "3 = Remove imported data / restore defaults\n\n" +
      "Enter 1-3:"
    );

    if(choice === "1"){ input.click(); return; }
    if(choice === "3"){ clearMenu(); return; }

    if(choice === "2"){
      const url = prompt("Paste your Google Sheet link (must be public / published):");
      if(!url) return;
      const csvUrl = sheetUrlToCsv(url.trim());
      try{
        ...
        console.error(err);
        alert("Failed to fetch sheet. Make sure it is PUBLIC and/or published to web.\n\n" + (err?.message || err));
      }
    }
  });
})();
